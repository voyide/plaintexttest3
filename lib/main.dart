...
class TestSession {
  final String id;
  String category; // Non-final to allow cascade renaming
  String subCategory; // Non-final to allow cascade renaming
  final int score;
...
class AppState extends ChangeNotifier {
  ...
  // Manipulators (With Cascading Deletes to wipe Graph/Profile Stats cleanly)
  void renameCategory(String oldName, String newName) {
    for (var q in _questions) { if (q.category == oldName) q.category = newName; }
    for (var s in _sessions) { if (s.category == oldName) s.category = newName; } // Cascade to sessions
    
    // Cascade to active states
  ...
  void renameSubCategory(String cat, String oldSub, String newSub) {
    ...
    for (var s in _sessions) { if (s.category == cat && s.subCategory == oldSub) s.subCategory = newSub; } // Cascade to sessions
  ...
  // CASCADING DELETIONS: Erases the test, the stats, the profile history.
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children:[
            ...
            TextField(controller: controller, maxLines: 12, decoration: const InputDecoration(hintText: 'PASTE_JSON_HERE...', labelText: 'DATA_INPUT')),
            const SizedBox(height: 16),
            FilledButton(
              onPressed: () async {
              ...
              itemBuilder: (context, idx) {
                ...
                return ListView(
                  ...
                  children:[
                    ...
                    const Divider(height: 32),
                    MarkdownBody(data: question.text, styleSheet: MarkdownStyleSheet(p: const TextStyle(fontSize: 18, height: 1.5, fontWeight: FontWeight.bold), code: TextStyle(backgroundColor: codeBg, color: codeText, fontFamily: 'monospace', fontSize: 16), codeblockDecoration: BoxDecoration(color: codeBg, borderRadius: BorderRadius.zero))),
                    const SizedBox(height: 32),
                    ...List.generate(question.options.length, (optIdx) {
                      ...
                      return InkWell(
                        ...
                        child: Container(
                          ...
                          child: Row(children:[Radio<int>(value: optIdx, groupValue: _answers[qId], activeColor: inkBlack, onChanged: (v) { setState(() => _answers[qId] = v!); _saveState(); }), Expanded(child: MarkdownBody(data: question.options[optIdx]))]),
                        ),
                      );
                    }),
                  ],
              ...
              return Card(
                ...
                child: Padding(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children:[
                    ...
                    const Divider(height: 24), MarkdownBody(data: q.text), const SizedBox(height: 16),
                    ...List.generate(q.options.length, (optIdx) {
                      ...
                      return Container(
                        ...
                        child: Row(children:[
                          ...
                          const SizedBox(width: 8), Expanded(child: MarkdownBody(data: q.options[optIdx])),
                        ]),
                      );
                    })
                  ]),
